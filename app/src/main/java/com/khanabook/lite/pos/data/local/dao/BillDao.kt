...
interface BillDao {
    ...
    @Query("SELECT * FROM bills WHERE daily_order_display = :displayId AND created_at LIKE :datePrefix || '%'")
    suspend fun getBillByDailyIdAndDate(displayId: String, datePrefix: String): BillEntity?

    @Query("SELECT * FROM bills WHERE daily_order_id = :dailyId AND created_at LIKE :datePrefix || '%'")
    ...
    @Query("SELECT * FROM bills WHERE created_at BETWEEN :startDate AND :endDate ORDER BY created_at DESC")
